const gameVersion = new URL(self.location.href).searchParams.get("v") || "v206";
...
const cacheableShellAssets = [
    ...
    "/images/War_Bird.png",
    "/images/logo.png",
    "/images/bulletdropnologo.png",
    "/images/bulletdropwlogo.png",
...
const cacheableNavigationPaths = [
    "/game-play",
    "/launcher",
    "/dashboard"
];

function pathIsInList(pathname, list) {
    return list.indexOf(pathname) !== -1;
}
...
function isLocalStaticAsset(pathname) {
    return pathname.startsWith("/build/assets/") ||
        ...
        pathname === "/game/manifest.webmanifest" ||
        pathname === "/game/icon-192.png" ||
        pathname === "/game/icon-512.png";
}
            ...
            return Promise.all(
                cacheableShellAssets.map(function (assetUrl) {
                    return fetch(assetUrl, {
                        ...
                        if (
                            pathIsInList(assetUrl, cacheableNavigationPaths) &&
...
self.addEventListener("fetch", function (event) {
    ...
    if (
        ...
        requestUrl.pathname === "/firebase-messaging-sw.js" ||
        requestUrl.pathname === "/game/firebase-notification.js"
    ) {
        event.respondWith(
            fetch(event.request, {
                cache: "no-store",
    ...
    if (isNavigation && pathIsInList(requestUrl.pathname, cacheableNavigationPaths)) {
        event.respondWith(
            caches.open(cacheName).then(function (cache) {
    ...
    if (isUnityBuildFile(requestUrl.pathname) || isLocalStaticAsset(requestUrl.pathname)) {
        event.respondWith(
            caches.open(cacheName).then(function (cache) {
